 ...
 * - The current segment doesn't end with sentence-ending punctuation (.!?)
 * - The time gap to the next segment is less than 1.5 seconds
 */
export function mergeSubtitleSegments(segments: SubtitleSegment[]): SubtitleSegment[] {
  ...
  for (let i = 1; i < segments.length; i++) {
    ...
    // If current segment doesn't end with punctuation and gap is small, merge
    if (!endsWithPunctuation && gap < 1.5) {

...
import { SubtitleSegment, VideoInfo, formatTime, translateSubtitles, getWordTimings } from '@/lib/api'
...
export default function LearnPage() {
  ...
  const updateCurrentSegment = useCallback((time: number) => {
    ...
    // First try exact match (time within segment range)
    let segmentIndex = videoInfo.subtitles.findIndex(
      (seg) => time >= seg.start && time < seg.end
    )

    // If no exact match, find the closest segment that started before current time
    if (segmentIndex === -1) {
      // Find the last segment whose start time is <= current time
      for (let i = videoInfo.subtitles.length - 1; i >= 0; i--) {
        if (videoInfo.subtitles[i].start <= time) {
          segmentIndex = i
          break
        }
      }
  ...
  const renderHighlightedText = (segment: SubtitleSegment, isCurrent: boolean) => {
    if (!isCurrent) {
      // Not current segment, just show plain text
      return <span>{segment.text}</span>
    }

    // Get word timings and highlight current word
    const wordTimings = getWordTimings(segment)
    
    return (
      <span>
        {wordTimings.map((wordTiming, idx) => {
          const isWordCurrent = currentTime >= wordTiming.start && currentTime < wordTiming.end
          const isWordPast = currentTime >= wordTiming.end
          
          return (
            <span
              key={idx}
              className={`
                inline-block px-1 py-0.5 rounded transition-all duration-150
                ${isWordCurrent 
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={`leading-relaxed ${isCurrent ? 'text-white font-medium' : 'text-gray-200'}`}>
                      {renderHighlightedText(segment, isCurrent)}
                    </p>
                    {/* Chinese translation */}
